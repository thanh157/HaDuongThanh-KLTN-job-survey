            ...
            <div class="form-section">
                ...
                <div class="row">
                    <div class="col-12 col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" value="{{$response->gender == 'male' ? 'Nam' : 'Nữ' }}"
                               readonly>
                    </div>
                    <div class="col-12 col-md-6 mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" value="{{ $response->code_student }}"
                           placeholder="Nhập mã sinh viên" readonly>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" readonly
                               value="{{ !empty($major[$response->training_industry_id]) ? $major[$response->training_industry_id] : "" }}">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" placeholder="Nhập số điện thoại"
                               value="{{ $response->phone_number }}" readonly>
                    </div>
                    <div class="col-12 col-md-6 mb-3">
                        <label class="form-label">9. Email</label>
                        <input type="email" class="form-control" placeholder="Nhập email" value="{{ $response->email }}"
                               readonly>
                    </div>
                </div>
                    ...
                    @foreach ($tinh_trang as $index => $value)
                        <div class="form-check mb-2">
                            <input class="form-check-input" disabled type="radio" name="employment_status"
                                   id="tt_{{ $index }}"
                                   value="{{ $index }}" {{ $index == $response->employment_status ? "checked" : "" }}>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Nhập tên công ty / tổ chức"
                           value="{{ $response->recruit_partner_name }}" readonly>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control" value="{{ $response->recruit_partner_date }}"
                           name="recruit_partner_date" readonly>
                </div>

                <div class="mb-3">
                    <label class="form-label">14. Chức vụ, vị trí việc làm</label>
                    <input type="text" class="form-control"
                           placeholder="VD: Nhân viên kinh doanh, Trưởng phòng sale..." readonly
                           name="recruit_partner_position" value="{{ $response->recruit_partner_position }}">
                </div>
            </div>
                ...
                @foreach (config('config.work_area') as $index => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="radio" name="work_area"
                               id="kv_{{ $index }}" value="{{ $index }}"
                               disabled {{ $response->work_area == $index ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach (config('config.employed_since') as $key => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="radio" name="employed_since"
                               ...
                               disabled {{ $response->employed_since == $key ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach (config('config.trained_field') as $key => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="radio" name="trained_field"
                               ...
                               disabled {{ $response->trained_field == $key ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach (config('config.professional_qualification_field') as $key => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="radio" name="professional_qualification_field"
                               ...
                               disabled {{ $response->professional_qualification_field == $key ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach (config('config.level_knowledge_acquired') as $key => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="radio" name="level_knowledge_acquired"
                               ...
                               disabled {{ $response->level_knowledge_acquired == $key ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach ($tim_viec as $index => $value)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="checkbox" name="recruitment_type[]" disabled
                               id="ht_{{ $index }}"
                               value="{{ $index }}" {{ in_array($index, $recruit['value']) ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach (config('config.job_search_method') as $key => $item)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="checkbox" name="job_search_method" disabled
                               id="ht_tuyen_{{ $key }}"
                               value="{{ $key }}" {{ in_array($key, $job_search_method_value['value']) ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
                ...
                @foreach ($ky_nang as $index => $value)
                    <div class="form-check mb-2">
                        <input class="form-check-input" type="checkbox" name="soft_skills_required[]" disabled
                               id="kn_{{ $index }}"
                               value="{{ $index }}" {{ in_array($index, $soft_skills_required['value']) ? "checked" : "" }}>
                        <label class="form-check-label fw-normal"
